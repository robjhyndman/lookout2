...
\begin{proof}
	...
	Note that for large enough $n$ this will lie in $(0,c/2M)$. By Lemma \ref{lem:covernumber} there exists a cover of $L$ by $l$ balls $\{B(y_i, \epsilon(n))\}$, with
	...
	If $Z_i$ is the event that at there are no samples in $B(y_i, \epsilon)$. By Lemma \ref{lem:samplescover}
...
\end{corollary}
\begin{proof}
	By Theorem \ref{thm:d_alphan to 0} we have $\lim_{n\to \infty} d_{\alpha(n-1)} =0$.

	As $f$ is a Lipschitz probability function it must be bounded. If $d=2$ then by Theorem \ref{thm:nd_f(n) in plane}, or if $d>2$ then by Theorem \ref{thm:nd_f(n) in higher dimensions}, we know that $\lim_{n\to \infty} nd_{\alpha(n-1)}=\infty$.
\end{proof}
