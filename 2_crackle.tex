...
\begin{proof}
	...
	Each death time corresponds to the reduction by one of the number of connected components. This implies $d_{\omega(n)} > \frac{r(n)}{n}$ occurs when $C(n, \frac{r(n)}{n})> n - \omega(n)$.
	...
	$\mathbb{E}[C(n, \frac{r(n)}{n})]\geq n- K \pi\log(\omega(n))$ and thus
	$$
		\lim_{n\to \infty} \frac{\mathbb{E}[C(n, \frac{r(n)}{n})]-n+\omega(n)}{\omega(n))} \geq \lim_{n\to \infty} \frac{\omega(n)-K \pi\log(\omega(n))}{\omega(n)}= 1.
	$$
\end{proof}
...
\subsection{Limit of $d_{\alpha (n-1)}$ (fixed percentiles of death times)}
...
\begin{theorem}\label{thm:d_alphan to 0}
	Let $f:\R^d \to \R$ be a Lipschitz probability density function for $X$. We want to show that $d_{\alpha (n-1)} \to 0$ where $d_{\alpha (n-1)}$ is the $\alpha$ percentile of finite death times.
\end{theorem}
...
\begin{corollary}\label{cor:consistency}
	Let $f:\R^d \to \R$ be a Lipschitz probability density function for $X$ such that . Fix $\alpha\in (0,1)$. Then $d_{\alpha (n-1)} \to 0$ (where $d_{\alpha (n-1)}$ is the $\alpha$ percentile of finite death times) is a consistent....
\end{corollary}
\begin{proof}
