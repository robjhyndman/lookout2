...
If we wished to prove that $d_*$ is a consistent estimator the only remaining task is to show $i(n) \to \infty$ at $n \to \infty$ almost surely. This seems intuitively clear but technically challenging. It is not the focus of the new version of lookout and thus we will leave this proof as an open problem.

\subsection{Counterexample for distributions with fat tails}

Within \cite{Crackle} the authors prove various limit theorems of lower bounds on the expected number of connected components of $\cup_{x}B(x,1)$ for $x$ sampled from exponential or power distributions over $\R^d$. This lower bound comes from bounding the number of isolated samples outside the ball $B(0,R_n)$ for well chosen $R_n$.  We can use these to show that $d_{g(n)}$ does not go to $0$ almost surely for large enough functions $g(n)$.
...
There are many choices of function $f(n)$ to decide which death time to use as a bandwidth. We wish for this choice to determine a consistent bandwidth parameter. This means we need $d_{f(n)} \to 0$ as $n \to \infty$.
...
\begin{lemma}
	Let $f: \R^d\to [0,\infty)$ be a Lipschitz probability density function (constant $M$), and $L=f^{-1} [c,\infty)$ be a non-empty superlevel set of $f$ for some $c>0$.  For all $\epsilon\in(0, c/2M]$ there there exists $\{x_1, \ldots x_l\}\subset L$ such that
...
\begin{proof}
	...
	This implies that as $n\to \infty$ we have at least one sample in each of the $B(y_i, \epsilon(n))$. All points with the same ball $B(y_i, \epsilon(n))$ will be connected in the Rips complex with radius $2\epsilon(n)$. This implies that the number of connected components when restricting to points inside of $L$ will be bounded above by $\frac{2^d n}{\log n}$ (almost surely).
