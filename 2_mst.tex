...
\cref{prop:order} does not cover the longest edge $e_{(n-1)}$, whose limiting distribution is known for uniform points in the unit square \citep{Penrose1997}, for normally distributed points \citep{Penrose1998}, and for a range of distributions on $\R^2$ with unbounded support \citep{HsingRootzen2005}. In each case the largest nearest-neighbour distance obeys the same limit law.
...
The proof in \cref{app:proofs} is short. With probability tending to one, more than $n^{1/4}/4$ observations lie beyond radius $R_n\asymp n^{3/4}$, and no two observations beyond radius $R_n-n^{1/8}$ lie within distance $n^{1/8}$ of each other. Each observation beyond $R_n$ is then a component on its own when observations are joined at distance $n^{1/8}$, so $C_n(n^{1/8})>n-\omega_n$: fewer than $\omega_n$ edges of $T$ have length at most $n^{1/8}$, and $e_{(j)}>n^{1/8}$ for every $j\ge\omega_n$. This is the crackle of \citet{Crackle}: heavy tails throw off isolated observations at ever greater distances, and the longest edges of $T$ record them. A fixed quantile never reaches that far, and \cref{prop:order} gives $h_n$ of order $n^{-1/m}$ for this density as for any other.

Light tails do not rescue the largest-gap rule. For Gaussian observations in $\R^m$ with $m\ge2$, the longest edge of $T$ is of order $\log\log n/\sqrt{\log n}$ \citep{Penrose1998}. That does tend to zero, but far more slowly than $n^{-1/m}$, so a rule that chooses among the longest edges has the right order only if the gap it finds sits well below the top of the edge-length distribution.
...
The guarantee to demand is therefore not consistency and not an optimal rate, but the correct order: $h_n$ should be the scale at which neighbouring observations sit, which is $n^{-1/m}$. That is what \cref{prop:order} delivers, and what the largest-gap rule puts at risk: that rule typically chooses among the longest edges, and under the heavy tails of \cref{cex:fattails} those grow without bound. Correct order is a weaker requirement than MISE-optimality, and it is the one that discriminates between the two rules.
