...
The proof is in \cref{app:proofs}. No smoothness, boundedness or tail condition on $f$ is required. Equation \eqref{eq:qgamma} is the single linkage interpretation in the limit: after rescaling distances by $n^{1/m}$, $q_\gamma(f)$ is the scale at which a large sample from $f$ has $1-\gamma$ components per observation.
