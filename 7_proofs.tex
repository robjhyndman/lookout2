...
Sections S1 to S3 of this appendix prove Proposition 1 and Counterexample 1 of the main paper, restated below; Section S4 reports additional experiments. Throughout, $\bm{y}_1,\dots,\bm{y}_n$ are sampled independently from a density $f$ on $\R^m$, $T$ is their Euclidean minimum spanning tree with ordered edge lengths $e_{(1)}\le\cdots\le e_{(n-1)}$, and $h_n=e_{(\lceil\gamma(n-1)\rceil)}$ for a fixed $\gamma\in(0,1)$. For $r>0$, $C_n(r)$ is the number of connected components of the graph that joins every pair of observations at distance at most $r$. The edges of $T$ of length at most $r$ form a spanning forest of that graph, so there are $n-C_n(r)$ of them, and $e_{(k)}\le r$ exactly when $C_n(r)\le n-k$. The ordered nearest-neighbour distances are $D_{(1)}\le\cdots\le D_{(n)}$. For $\lambda>0$, $\mathcal{P}_\lambda$ is a homogeneous Poisson process of intensity $\lambda$ on $\R^m$; joining each pair of points of $\mathcal{P}_\lambda\cup\{\bm{0}\}$ at distance at most $1$, $\mathcal{C}_\lambda$ is the component containing the origin, and $\kappa(\lambda)=\E[1/|\mathcal{C}_\lambda|]$, with $1/|\mathcal{C}_\lambda|=0$ when the component is infinite. Finally, $b_m$ denotes the volume of the unit ball in $\R^m$, $s_{m-1}=mb_m$ the $(m-1)$-dimensional surface measure of the unit sphere $S^{m-1}\subset\R^m$, and $B(\bm{x},r)$ the closed ball of radius $r$ centred at $\bm{x}$.
...
Write $\mathcal{Y}_n=\{\bm{y}_1,\dots,\bm{y}_n\}$ and $k_n=\lceil\gamma(n-1)\rceil$.
...
\begin{proof}[Proof of \cref{prop:order}]
  ...
  $$
  in probability. The left side is the proportion of observations whose nearest neighbour lies within $tn^{-1/m}$, so the argument of Step 3, with $G_-$ in place of $G$, gives $n^{1/m}D_{(k_n)}\to t_-$ in probability, as stated after Proposition 1 in the main paper.
\end{proof}
