...
Let $f(\bm{x})=c_m/(1+\|\bm{x}\|^{m+1})$ on $\R^m$ with $m\ge1$, let $\bm{Y}$ have density $f$, and write $a_m=c_ms_{m-1}$. Two elementary bounds drive the proof. First, $\|\bm{Y}\|$ has density $a_mr^{m-1}/(1+r^{m+1})$, which for $r\ge1$ lies between $a_m/(2r^2)$ and $a_m/r^2$, so
...
\begin{proof}[Proof of \cref{cex:fattails}]
  ...
  \emph{Enough observations far out.} $N_n$ is binomial with $n$ trials and success probability $p(R_n)$, which by \eqref{eq:tailprob} lies in $[n^{-3/4}/2,\,n^{-3/4}]$. Its mean $\mu_n$ therefore satisfies $n^{1/4}/2\le\mu_n\le n^{1/4}$, its variance is at most $\mu_n$, and $K_n\le\mu_n/2$. By Chebyshev's inequality,
  $$
  \P(N_n\le K_n)\le\P\big(|N_n-\mu_n|\ge\mu_n/2\big)\le\frac{4}{\mu_n}\le8n^{-1/4}\to0.
  $$

  \emph{No close pairs far out.} Conditioning on the position of the first point of a pair and applying \eqref{eq:tailprob} and \eqref{eq:ballprob},
  $$
  ...
  \emph{Conclusion.} Suppose $N_n>K_n$ and $Y_n=0$. Any observation within distance $s_n$ of an observation $\bm{y}_i$ with $\|\bm{y}_i\|\ge R_n$ has norm at least $R_n-s_n$, so the two would form a pair counted by $Y_n$; there is none. Each of the $N_n$ observations beyond $R_n$ is therefore a component on its own when observations are joined at distance $s_n$, so $C_n(s_n)\ge N_n>K_n=n-\omega_n$. By \cref{sec:order}, exactly $n-C_n(s_n)$ edges of $T$ have length at most $s_n$, which is fewer than $\omega_n$, so $e_{(\omega_n)}>s_n$. Therefore
  \begin{equation*}
