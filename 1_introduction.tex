...
\State Compute the persistence homology barcode of the data cloud for dimension zero using the Vietoris-Rips diameter \citep{ghrist2008barcodes}.
...
\State Compute the kernel density estimates at the observations: $f_i = \hat{f}(\bm{y}_i; \bm{H})$, $i=1,\dots,n$, where $\hat{f} (\bm{y}; \bm{H}) = \frac{1}{n} \sum_{j = 1}^n K_{\bm{H}}(\bm{y} - \bm{y}_j)$, $\bm{H} = (d_*)^{2/p} \bm{I}_p$, $K_{\bm{H}}(\bm{u}) = |\bm{H}|^{-1/2} K(\bm{H}^{-1/2} \bm{u})$, and $K$ is a multivariate kernel function \citep{chacon2018multivariate}.
...
\end{equation}
In the following section, we will explore the conditions under which these hold for $d(n)=d_*$, and the implications for the lookout algorithm. In particular, we will show that the conditions are not satisfied for distributions with heavy tails such as a Gamma distribution, although it seems likely that they are satisfied for lighter-tailed distributions such as the Gaussian distribution. We will also show that an alternative choice for $d(n)$, based on an upper quantile of the Rips death diameters, satisfies these conditions for a wide range of distributions.
