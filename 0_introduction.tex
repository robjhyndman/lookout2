...
\begin{defn}
We say that $h_n$ is an \emph{admissible bandwidth} if $(h_n)^{2/p}\rightarrow 0$ and $nh_n\rightarrow \infty$ as $n\rightarrow\infty$ almost surely.
\end{defn}
Thus, when $\bm{H} = [h_n]^{2/p} \bm{I}_p$ where $h_n\in\R$ is a function of the sample $\{\bm{y}_1,\dots,\bm{y}_n\}$ which is an admissible bandwidth then
$\bm{H} = [h_n]^{2/p} \bm{I}_p$ is a consistent kernel density estimator.


In the following section, we will explore the conditions under which these hold for $h_n=d_*$, and the implications for the lookout algorithm. In particular, we will show that the conditions are not satisfied for distributions with heavy tails such as a power distribution, although it seems likely that they are satisfied for lighter-tailed distributions such as the Gaussian distribution. We will also show that an alternative choice of bandwidth, based on an upper quantile of the Rips death diameters, satisfies these conditions for a wide range of distributions.
